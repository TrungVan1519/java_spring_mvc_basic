...
public class SpringSecurityAppConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.jdbcAuthentication().dataSource(dataSource)
	        .usersByUsernameQuery("SELECT `username`, `password`, `enabled` FROM `plaintext_account` WHERE `username` = ?")
	        .authoritiesByUsernameQuery(
	        		"SELECT plaintext_account.username, plaintext_authority.authority "
	        		+ "FROM plaintext_account, plaintext_authority "
	        		+ "WHERE plaintext_account.username = ? "
	        		+ "AND plaintext_account.id = plaintext_authority.account_id");
	}
